...
def get_thermal_stats() -> Dict[str, Optional[float]]:
    """
    Read CPU / GPU / NPU temperatures from Linux thermal zones
    (/sys/class/thermal/thermal_zone*/{type,temp}). Generic sysfs, not
    vendor-specific - matches this Jetson's zone names (cpu-thermal,
    gpu-thermal, ...) as well as Qualcomm's (cpu-*-thermal, gpuss-*-thermal,
    nsp-*-thermal for the Hexagon NPU).

    Each group is averaged across however many zones match. Returns None for
    any group with no matching zones (e.g. this board has no "nsp-" NPU zone)
    so callers can show "N/A" gracefully.
    """
    ...
    def get_stats(self) -> Dict:
        ...
        if self.use_jtop and self.jtop_instance:
            try:
                ...
                # jetson-stats >=6 returns a Memory object that is dict-like
                # (supports .get()/.keys()/.items()) but is not a dict subclass
                is_dict_like = isinstance(memory, dict) or hasattr(memory, "get")
                if is_dict_like:
                ...
                if vram_total_gb == 0 and not hasattr(self, "_vram_warning_logged"):
                    logger.warning(
                        f"VRAM total is 0 - jtop memory structure: {type(memory)}, keys: {list(memory.keys()) if is_dict_like else 'N/A'}"
                    )
                ...
                # Only fall back to nvidia-smi if jtop gave us structurally invalid
                # data (missing VRAM total or wrong types) - GPU=0% is a legitimate
                # idle reading on Jetson, not a sign of broken monitoring. Note
                # nvidia-smi doesn't report real utilization/memory on Jetson anyway
                # (unified memory, no discrete GPU), so this fallback rarely helps here.
                if (
                    vram_total_gb == 0
